...
const CustomerReview = () => {
    ...
    return (
        <div className="flex flex-col items-center bg-custom-gradient py-12 px-4">
            <h1 className="text-3xl font-bold mb-8">Customer Reviews</h1>
            {error && <p className="text-red-500 mb-4">{error}</p>}
            <div className="flex flex-wrap justify-center gap-20">
                {currentFeedbacks.map((item, index) => (
                    <div key={index}
                        className={`bg-white p-6 flex flex-col items-center space-y-4 rounded-lg shadow-lg transform transition-transform duration-500
